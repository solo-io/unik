...
type VirtualMachine2 struct {
	...
	Config struct {
		...
		Tools struct {
			ToolsVersion         int64       `json:"ToolsVersion"`
		...
		CPUAllocation struct {
			Reservation           int         `json:"Reservation"`
			ExpandableReservation interface{} `json:"ExpandableReservation"`
			Limit                 int         `json:"Limit"`
			Shares                struct {
				Shares int    `json:"Shares"`
				Level  string `json:"Level"`
	...
	Guest struct {
		...
		Net                 []struct {
			...
			DeviceConfigID int         `json:"DeviceConfigId"`
			...
			NetBIOSConfig interface{} `json:"NetBIOSConfig"`
		} `json:"Net"`
		IPStack []struct {
			DNSConfig struct {
				Dhcp         bool     `json:"Dhcp"`
			...
			IPStackConfig interface{} `json:"IpStackConfig"`
			DhcpConfig    interface{} `json:"DhcpConfig"`
		} `json:"IpStack"`
		Disk []struct {
